...
\subsection{Comparison with the Relativistic Characteristic Timescale}
...
The timescale $T$ obtained in Section~2.3, despite being derived from a
Newtonian-inspired and physically inconsistent construction, is dimensionally
comparable to this relativistic characteristic timescale. This comparison is
instructive: it demonstrates that even naive manipulations of the Schwarzschild
condition can reproduce the correct \emph{order of magnitude} for black-hole
timescales, while still failing to capture the underlying spacetime geometry
responsible for such behavior.

This agreement in scale should not be interpreted as validation of the
derivation presented in Section~2.3. Rather, it reflects the fact that the
Schwarzschild radius encodes a fundamental relativistic scale that can appear
in multiple mathematical contexts. The comparison therefore reinforces the
central theme of this work: mathematical consistency alone does not guarantee
physical meaning, and only a fully relativistic framework correctly describes
photon behavior and causal structure near an event horizon.
